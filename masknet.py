...
def create_model(image_size):
    ...
    P5 = KL.Conv2D(64, (1, 1), name='fpn_c5')(C5)
    ...
    P2 = KL.Conv2D(64, (3, 3), padding="SAME", name="fpn_p2")(P2)
    P3 = KL.Conv2D(64, (3, 3), padding="SAME", name="fpn_p3")(P3)
    P4 = KL.Conv2D(64, (3, 3), padding="SAME", name="fpn_p4")(P4)
    P5 = KL.Conv2D(64, (3, 3), padding="SAME", name="fpn_p5")(P5)

    feature_maps = [P2, P3, P4, P5]

    roi_pool_layer = PyramidROIAlign((my_msk_inp, my_msk_inp), image_size,
                                     name="roi_align_mask")([roi_input] + feature_maps)

    x = KL.TimeDistributed(KL.Conv2D(64, (3, 3), padding="same"), name="mrcnn_mask_conv1")(roi_pool_layer)
    x = KL.TimeDistributed(BatchNorm(axis=3, name='mrcnn_mask_bn1'))(x)
    x = KL.Activation('relu')(x)

    x = KL.TimeDistributed(KL.Conv2D(64, (3, 3), padding="same"), name="mrcnn_mask_conv2")(x)
    x = KL.TimeDistributed(BatchNorm(axis=3), name='mrcnn_mask_bn2')(x)
    x = KL.Activation('relu')(x)

    x = KL.TimeDistributed(KL.Conv2D(64, (3, 3), padding="same"), name="mrcnn_mask_conv3")(x)
    x = KL.TimeDistributed(BatchNorm(axis=3), name='mrcnn_mask_bn3')(x)
    x = KL.Activation('relu')(x)

    x = KL.TimeDistributed(KL.Conv2D(64, (3, 3), padding="same"), name="mrcnn_mask_conv4")(x)
